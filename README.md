...
The plugin checks both command presence and the required JSON capabilities as
soon as its collector starts and again before every collection. Missing or
...
Noctalia v5 treats `~/.local/share/noctalia/plugins` as its highest-precedence
local plugin source. Link this checkout there, enable it, then add the summary
entry to a bar:

```sh
mkdir -p ~/.local/share/noctalia/plugins
ln -s ~/src/noctalia-drive-health-plugin \
  ~/.local/share/noctalia/plugins/smart-monitor
noctalia msg plugins enable gustav0ar/drive-health-plugin
```

The bar entry type is `gustav0ar/drive-health-plugin:summary`; clicking it toggles the
...
Noctalia. The Noctalia process never receives root access and never invokes
`sudo`:
...
Noctalia lint (when available), and whitespace errors. The same target runs in
GitHub Actions.
